...
func (s *InteractiveTestSuite) TearDownSuite() {
	...
	err = s.db.Exec("ALTER SEQUENCE interactives_id_seq RESTART WITH 1").Error
	assert.NoError(s.T(), err)
	err = s.db.Exec("TRUNCATE TABLE public.user_like_bizs").Error

	assert.NoError(s.T(), err)
	err = s.db.Exec("ALTER SEQUENCE webook.public.user_like_bizs_id_seq RESTART WITH 1").Error
	assert.NoError(s.T(), err)
}
...
func (s *InteractiveTestSuite) TestLike() {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.before(t)
			err := svc.Like(context.Background(), tc.biz, tc.bizId, tc.user_id)
			assert.NoError(t, err)
			tc.after(t)
		})
	}
...
func (s *InteractiveTestSuite) TestDislike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.before(t)
			err := svc.CancelLike(context.Background(), tc.biz, tc.bizId, tc.user_id)
			assert.NoError(t, err)
			tc.after(t)
		})
	}
